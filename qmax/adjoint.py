...
def _propagate_bwd(res, grad_out, _, vjp_args, U, running_cost_fn, save_every, save_fn, *args, **kwargs):
    ...
    if jax.tree.leaves(g_ys):
        # On forward pass, the last save happens outside the loop
    ...
    def bwd_step(bwd_carry, args):
        carry_next, g_carry_next = bwd_carry
    ...
    def bwd_loop(bwd_carry, args):
        ...
        out, vjp = eqx.filter_vjp(lambda yy, uu: save_fn(t_starts[0], yy, uu), y, u_starts[0])

        # in case g_save has any leaves that are None
        g_y_step, g_u_save = vjp(eqx.combine(g_save, jax.tree.map(jnp.zeros_like, out)))

        g_y, g_cost, g_total = g_carry
        g_y = g_y + g_y_step
    ...
    if not jax.tree.leaves(g_ys):
        # bypass outer loop if we're not differentating with respect to ys
    ...
    if jax.tree.leaves(g_ys):
        ...
        g_us = g_us.at[::save_every].add(g_u_saves)
