...
# TODO:
#   Alternative adapation schemes evaluating t over range [t0, t1]
#   for timestepping and exponential integrators
...
# Backward-error parameters for the scaled and truncated Taylor scheme of
# A. H. Al-Mohy and N. J. Higham (2011), Table 3.1. THETA_TABLES[tol][m - 1] is
# the largest ||h (A - mu I)|| for which the degree-m truncated Taylor polynomial
# has backward error <= tol.
#
# Generated by utils/mohy_backward_error.py
...
def hutchinson(
    operator: Operator, num_samples: int = 10, key: Optional[PRNGKeyArray] = None
) -> Scalar:
    """
...
def _top_indices(h: Array, num_indices: int, exclude: Optional[Array] = None) -> Array:
    ...
    excluding those indices in exclude. Runs in O(num_indices * n) time, so when
    num_indices is small compared to h.shape[0], this is faster than argsort(h)[:num_indices],
    which is O(n log n).
    """
...
def block_estimate(
    ...
    for _ in range(num_iterations - 1):
        ...
        indices = _top_indices(h, num_samples, exclude=jnp.concatenate(used) if used else None)
        used.append(indices)
...
def _taylor_expm(
    op: Operator, h: ComplexScalarLike, y: AbstractState, num_iterations: int
) -> AbstractState:
    """
    Computes T_k(A) @ y, where T_k is the kth order Taylor expansion of e^x.
    """
...
class TruncatedTaylorExponentiator(AbstractExponentiator):
    ...
    s: Optional[int] = eqx.field(
        static=True, default=None, converter=lambda s: s if s is None else int(s)
    )
    ...
    def adapt(self, op: Operator, dt_max: ScalarLike) -> AbstractExponentiator:
        ...
        try:
            ...
            theta1 = block_estimate(
                ...
                num_samples=NUM_BLOCK_SAMPLES,
                num_iterations=NUM_BLOCK_ITERATIONS,
            )
        ...
        if theta < 2 * NUM_BLOCK_SAMPLES * theta_list[-1] / M_MAX * P_MAX * (P_MAX + 3):
            ...
            admissible = (p_list[:, None] * (p_list[:, None] - 1) - 1 <= m_list[None, :]) & (
                m_list[None, :] >= 2
            )
